"""
Modified version of app.py that attempts to connect to MongoDB Atlas but falls back to local MongoDB.
This is a solution to handle SSL/TLS issues with MongoDB Atlas.
"""
...
try:
    ...
    try:
        print("Trying MongoDB Atlas connection...")
        atlas_uri = 'mongodb+srv://kavin88701:nR1Cc1SOsgRiXedQ@cluster0.01myarq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'
        
        # Try different connection approaches for Atlas
        try:
            ...
            print(f"Standard Atlas connection failed: {str(e1)}")
            
            try:
                # Approach 2: With TLS CA file
                client = MongoClient(
                    atlas_uri,
                    tlsCAFile=certifi.where()
                )
                client.admin.command('ping')
                print("Connected to MongoDB Atlas with TLS CA file")
            except Exception as e2:
                print(f"TLS CA file Atlas connection failed: {str(e2)}")
                
                try:
                    # Approach 3: With TLS disabled
                    client = MongoClient(
                        atlas_uri,
                        tls=True,
                    ...
                    client.admin.command('ping')
                    print("Connected to MongoDB Atlas with TLS disabled")
                except Exception as e3:
                    print(f"TLS disabled Atlas connection failed: {str(e3)}")
                    
                    # If all Atlas approaches fail, fall back to local MongoDB
                    raise Exception("All MongoDB Atlas connection approaches failed")
        
        # If we get here, we successfully connected to MongoDB Atlas
        db = client.get_database(db_name)
        print(f"Using MongoDB Atlas database: {db_name}")
        
    except Exception as atlas_error:
        print(f"MongoDB Atlas connection failed: {str(atlas_error)}")
        print("Falling back to local MongoDB...")
        
        # Connect to local MongoDB as fallback
        local_uri = 'mongodb://localhost:27017/'
        client = MongoClient(local_uri, serverSelectionTimeoutMS=5000)
        db = client.get_database(db_name)
        print(f"Connected to local MongoDB database: {db_name}")
